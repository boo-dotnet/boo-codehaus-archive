# BOO-817: issue an error "event can only be invoked from within the class that declared it"

| Field | Value |
|---|---|
| Type | Bug |
| Status | Resolved |
| Priority | Minor |
| Resolution | Fixed |
| Reporter | George Dernovoy |
| Assignee | Cedric Vivier |
| Affects | 0.7.7 |
| Fix for | 0.8.1 |
| Components | Compiler |
| Created | 2007-04-14T17:02-0500 |
| Updated | 2008-02-07T07:04-0600 |
| Resolved | 2008-02-07T07:04-0600 |

## Description

discussion: http://groups.google.com/group/boolang/browse_thread/thread/a553505834526d03

import System

AppDomain().ProcessExit()

should issue an error "event can only be invoked from within the class that declared it"

instead of an internal compiler error.

## Comments (2)

### comment-122752

Hide

 Permalink

 Cedric Vivier
 added a comment - 07/Feb/08 7:04 AM 

fixed in rev. 2854

boo now only allows raising an event from its declaring class. documentation changed to clearer description. 

 Show

 Cedric Vivier
 added a comment - 07/Feb/08 7:04 AM fixed in rev. 2854 

 boo now only allows raising an event from its declaring class. documentation changed to clearer description.

### comment-122557

Hide

 Permalink

 Cedric Vivier
 added a comment - 06/Feb/08 4:38 AM 

It's almost done but I've noticed something I'm not sure what to do (or not do) with it :

Currently we allow invoking base class events from derived classes.

In our documentation we don't really say it is the case (or very implied) :

 "It can't be called from outside its parent class." 

C# is clearer about this :

"Derived classes cannot directly invoke events that are declared within the base class."

Should we fix the documentation or only allow invoking events from the declaring class ?

 Show

 Cedric Vivier
 added a comment - 06/Feb/08 4:38 AM It's almost done but I've noticed something I'm not sure what to do (or not do) with it : 

 Currently we allow invoking base class events from derived classes. 
In our documentation we don't really say it is the case (or very implied) : 

 "It can't be called from outside its parent class." 

 C# is clearer about this : 

 "Derived classes cannot directly invoke events that are declared within the base class." 

 Should we fix the documentation or only allow invoking events from the declaring class ?

## Provenance

- <http://web.archive.org/web/20150511142312id_/https://jira.codehaus.org/browse/BOO-817>
- <http://web.archive.org/web/20150517100938id_/https://jira.codehaus.org/browse/BOO-817?page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel>
