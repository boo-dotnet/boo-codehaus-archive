# BOO-995: Add the "or" keyword semantic for for/while loops

| Field | Value |
|---|---|
| Type | New Feature |
| Status | Reopened |
| Priority | Minor |
| Resolution | Unresolved |
| Reporter | Marcus Griep |
| Assignee | Marcus Griep |
| Affects | 0.8.1 |
| Fix for | 0.8.2 |
| Components | Compiler |
| Created | 2008-04-12T07:57-0500 |
| Updated | 2008-10-17T06:50-0500 |

## Description

Allow "or" as in:

```
"""
No items!
"""
t = []

for item in t:
 print item
or:
 print "No items!"

```

"Or" executes its block of code only when the iterator (in for loops) or condition (in while loops) evaluates to empty or false, respectively, on the first test, i.e. before entering the loop. If control enters the loop, then the "or" block will not be executed.

"Break" and "continue" are valid within an "or" block. When no "then" block exists, they convey the same semantic, passing control to the statement after the or block. When a "then" block exists, "break" passes control to the statement after the "then" block, whereas "continue" passes control to the "then" block. The idea here being that if the iterator/condition is empty/false on a "continue", then exit the loop via the "then" block as if the loop were executing.

This pseudo-IL indicates the execution flow without a "then" block:

```
flag = false
goto cond
:body
flag = true
&lt;loop_block&gt;
:cond
if &lt;cond&gt; goto body
if flag goto end
&lt;or_block&gt;
:end

```

With a "then" block the pseudo-IL is as follows:

```
flag = false
goto cond
:body
flag = true
&lt;loop_block&gt;
:cond
if &lt;cond&gt; goto body
if flag goto end
&lt;or_block&gt;
:then
&lt;then_block&gt;
:end

```

## Comments (6)

### comment-151118

Hide

 Permalink

 Rodrigo B. de Oliveira
 added a comment - 17/Oct/08 6:50 AM 

Cedric, it looks ok to me.

Those pseudo IL snippets are basically boo code templates, why not implement those transformations in boo code? 

 Show

 Rodrigo B. de Oliveira
 added a comment - 17/Oct/08 6:50 AM Cedric, it looks ok to me. 

 Those pseudo IL snippets are basically boo code templates, why not implement those transformations in boo code?

### comment-151104

Hide

 Permalink

 Cedric Vivier
 added a comment - 17/Oct/08 3:16 AM 

So, is there a consensus about my comment above ? 

 Show

 Cedric Vivier
 added a comment - 17/Oct/08 3:16 AM So, is there a consensus about my comment above ?

### comment-137439

Hide

 Permalink

 Aleksandr Dubinsky
 added a comment - 04/Jun/08 9:15 PM 

would it be in the spirit of Cdric's suggestion and nearly as helpful to just never throw NullRefException from for? 

 Show

 Aleksandr Dubinsky
 added a comment - 04/Jun/08 9:15 PM would it be in the spirit of Cdric's suggestion and nearly as helpful to just never throw NullRefException from for?

### comment-130744

Hide

 Permalink

 Cedric Vivier
 added a comment - 14/Apr/08 9:16 AM 

Yes as in this pseudo-IL.

Imho this adds more value and potential usages to the 'or' construct while being consistent ('or' is executed when you do not enter the loop).

What do you think? Rodrigo? Avish?

Also I guess this doesn't deserve a separate ticket for itself, in some way it is still about the addition of the 'or' keyword which haven't been in a release yet, but do as you prefer of course.

 Show

 Cedric Vivier
 added a comment - 14/Apr/08 9:16 AM Yes as in this pseudo-IL. 
Imho this adds more value and potential usages to the 'or' construct while being consistent ('or' is executed when you do not enter the loop). 
What do you think? Rodrigo? Avish? 

 Also I guess this doesn't deserve a separate ticket for itself, in some way it is still about the addition of the 'or' keyword which haven't been in a release yet, but do as you prefer of course.

### comment-130743

Hide

 Permalink

 Marcus Griep
 added a comment - 14/Apr/08 8:59 AM - edited 

As in this pseudo-IL?

```

if &lt;iterator&gt; == null goto or
&lt;enumerator&gt; = &lt;iterator&gt;.GetEnumerator()
flag = false
goto cond
:body
 flag = true
 &lt;loop_block&gt;
:cond
 if &lt;enumerator&gt;.MoveNext() goto body
 if flag goto end
:or
 &lt;or_block&gt;
:then
 &lt;then_block&gt;
:end

```

If that's the case, I think it's a separate ticket. The real question is whether we should behave this way on null values. It wouldn't affect while, only the way in which for interprets a null iterator. 

 Show

 Marcus Griep
 added a comment - 14/Apr/08 8:59 AM - edited As in this pseudo-IL? 

 if &lt;iterator&gt; == null goto or
&lt;enumerator&gt; = &lt;iterator&gt;.GetEnumerator()
flag = false 
 goto cond
:body
 flag = true 
 &lt;loop_block&gt;
:cond
 if &lt;enumerator&gt;.MoveNext() goto body
 if flag goto end
:or
 &lt;or_block&gt;
:then
 &lt;then_block&gt;
:end

 If that's the case, I think it's a separate ticket. The real question is whether we should behave this way on null values. It wouldn't affect while , only the way in which for interprets a null iterator.

### comment-130655

Hide

 Permalink

 Cedric Vivier
 added a comment - 13/Apr/08 7:57 PM - edited 

```

list as List #let's say it's a member of a class for a more common usage
 
for i in list:
 print "in"
or:
 print "or"

```

This code 'obviously' results in a NullReferenceException.

However I think the 'or' construct should handle this nicely, as this match the semantic (loop is not entered) and would allow shrinking the common scenario below :

```

if list is not null:
 for i in list:
 ...
else:
 ...

```

Thoughts? 

 Show

 Cedric Vivier
 added a comment - 13/Apr/08 7:57 PM - edited 
 
list as List #let's say it's a member of a class for a more common usage
 
 for i in list:
 print "in" 
or:
 print "or" 

 This code 'obviously' results in a NullReferenceException. 
However I think the 'or' construct should handle this nicely, as this match the semantic (loop is not entered) and would allow shrinking the common scenario below : 

 if list is not null :
 for i in list:
 ...
 else :
 ...

 Thoughts?

## Provenance

- <http://web.archive.org/web/20150511142253id_/https://jira.codehaus.org/browse/BOO-995>
- <http://web.archive.org/web/20150517100647id_/https://jira.codehaus.org/browse/BOO-995?page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel>
