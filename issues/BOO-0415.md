# BOO-415: Type member macros

| Field | Value |
|---|---|
| Type | Improvement |
| Status | Closed |
| Priority | Major |
| Resolution | Fixed |
| Reporter | Ayende Rahien |
| Assignee | Rodrigo B. de Oliveira |
| Affects | None |
| Fix for | 0.9 |
| Components | Parser |
| Created | 2005-08-02T01:29-0500 |
| Updated | 2009-01-26T19:13-0600 |
| Resolved | 2009-01-26T19:13-0600 |

## Description

Allow a way to specify class macros in a clean way.

Currently we've no way to specify a macro that affect a class. This lead to using AstAttrbiutes on classes.

When the class has several such attributes, it starts to get unsightly.

The issue rose when discussing class' contracts, btw. The proposed syntax is macros for the method contract, but there is no good way to specify a class contract without using an attribute (when you specify method's class with macros, that is not consistent).

I suggest a way to specify a macro on a class, similar to the way you can on methods. Considerring the possible confusion with properties, it would be nice to prefix them with @, so the following code is possible:

class MyClass: 

 @disposable #macro with no body, behave just like an attribute, with better syntax

 @invariant: #macro with body,

 var != null

 count &gt; 0

## Comments (4)

### comment-161373

Hide

 Permalink

 Rodrigo B. de Oliveira
 added a comment - 17/Jan/09 8:07 PM 

I'm currently considering demanding either a type or an initializer for a field. That way, the syntax for macros could be the same for types. 

 Show

 Rodrigo B. de Oliveira
 added a comment - 17/Jan/09 8:07 PM I'm currently considering demanding either a type or an initializer for a field. That way, the syntax for macros could be the same for types.

### comment-161375

Hide

 Permalink

 Ayende Rahien
 added a comment - 17/Jan/09 8:18 PM 

That make a lot of sense, there are rare cases for object fields on an class, so that is a low impact change. 

 Show

 Ayende Rahien
 added a comment - 17/Jan/09 8:18 PM That make a lot of sense, there are rare cases for object fields on an class, so that is a low impact change.

### comment-162466

Hide

 Permalink

 Rodrigo B. de Oliveira
 added a comment - 25/Jan/09 8:09 PM 

Implemented in revision 3194. 

 Show

 Rodrigo B. de Oliveira
 added a comment - 25/Jan/09 8:09 PM Implemented in revision 3194.

### comment-162603

Hide

 Permalink

 Rodrigo B. de Oliveira
 added a comment - 26/Jan/09 5:06 PM 

Remaining issues documented separately. 

 Show

 Rodrigo B. de Oliveira
 added a comment - 26/Jan/09 5:06 PM Remaining issues documented separately.

## Provenance

- <http://web.archive.org/web/20150515152411id_/http://jira.codehaus.org/browse/BOO-415>
- <http://web.archive.org/web/20150517102826id_/https://jira.codehaus.org/browse/BOO-415?page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel>
