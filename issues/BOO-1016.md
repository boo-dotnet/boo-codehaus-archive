# BOO-1016: Conflict between generic and non-generic methods overloads when return type is generic parameter

| Field | Value |
|---|---|
| Type | Bug |
| Status | Resolved |
| Priority | Major |
| Resolution | Fixed |
| Reporter | Spruce Weber |
| Assignee | Cedric Vivier |
| Affects | 0.8.1 |
| Fix for | 0.8.2 |
| Components | Compiler |
| Created | 2008-05-16T11:46-0500 |
| Updated | 2008-05-16T14:33-0500 |
| Resolved | 2008-05-16T14:33-0500 |

## Description

Two test cases and corresponding errors follows...

interface ITest:

 def DoSomething() as string

 def DoSomething[of T]() as T

class Test(ITest):

 def DoSomething() as string:

 pass

 def DoSomething[of T]() as T:

 pass

Program.boo(10,9) : Error BCE0035: 'Test.DoSomething' conflicts with inherited member 'ITest.DoSomething'.

Program.boo(7,9) : Error BCE0035: 'Test.DoSomething' conflicts with inherited member 'ITest.DoSomething`1'.

class Test2Base:

 virtual def DoSomething() as string:

 pass

 virtual def DoSomething[of T]() as T:

 pass

class Test2(Test2Base):

 def DoSomething() as string:

 pass

 def DoSomething[of T]() as T:

 pass

Program.boo(12,32) : Error BCE0072: Overridden method 'Test2Base.DoSomething' has a return type of 'string' not 'Test2.DoSomething`1.T'.

## Comments (5)

### comment-135163

Hide

 Permalink

 Cedric Vivier
 added a comment - 16/May/08 12:06 PM 

Hmm.. I guess the errors are legit aren't they?

```

interface ITest:
 def DoSomething() as string
 def DoSomething[of T](x as T) as string

class Test(ITest):
 def DoSomething() as string:
 pass
 def DoSomething[of T](x as T) as string:
 pass

```

Works.

 Show

 Cedric Vivier
 added a comment - 16/May/08 12:06 PM Hmm.. I guess the errors are legit aren't they? 

 interface ITest:
 def DoSomething() as string
 def DoSomething[of T](x as T) as string

class Test(ITest):
 def DoSomething() as string:
 pass
 def DoSomething[of T](x as T) as string:
 pass

 Works.

### comment-135165

Hide

 Permalink

 Spruce Weber
 added a comment - 16/May/08 12:27 PM 

I'm not sure they are. The C# equivalent seems to compile fine...

 public interface ITest
{
 string DoSomething();
 
 T DoSomething&lt;T&gt;();
 }

 public class Test:ITest

 {

 public string DoSomething()
{return null;}

 public T DoSomething&lt;T&gt;()
{return default(T);}

 }

By the way. What are the tags for the nice code formatting? 

 Show

 Spruce Weber
 added a comment - 16/May/08 12:27 PM I'm not sure they are. The C# equivalent seems to compile fine... 
 public interface ITest 
{
 string DoSomething();
 
 T DoSomething&lt;T&gt;();
 }

 public class Test:ITest 
 { 
 public string DoSomething() 
{return null;}

 public T DoSomething&lt;T&gt;() 
{return default(T);}
 } 

 By the way. What are the tags for the nice code formatting?

### comment-135167

Hide

 Permalink

 Cedric Vivier
 added a comment - 16/May/08 12:33 PM - edited 

Hmm true, so this is ok only when return types differ because of genericity?

surround your code with {{ code }} (without spaces) for the nice formatting.

 Show

 Cedric Vivier
 added a comment - 16/May/08 12:33 PM - edited Hmm true, so this is ok only when return types differ because of genericity? 

 surround your code with {{ code }} (without spaces) for the nice formatting.

### comment-135173

Hide

 Permalink

 Spruce Weber
 added a comment - 16/May/08 1:48 PM 

The C# version compiles regardless of whether or not the return types differ. Boo fails when the return types differ even when a non-generic type is returned from the generic method. 

 Show

 Spruce Weber
 added a comment - 16/May/08 1:48 PM The C# version compiles regardless of whether or not the return types differ. Boo fails when the return types differ even when a non-generic type is returned from the generic method.

### comment-135178

Hide

 Permalink

 Cedric Vivier
 added a comment - 16/May/08 2:33 PM 

Fixed in rev. 2952 

 Show

 Cedric Vivier
 added a comment - 16/May/08 2:33 PM Fixed in rev. 2952

## Provenance

- <http://web.archive.org/web/20150515152353id_/http://jira.codehaus.org/browse/BOO-1016>
- <http://web.archive.org/web/20150517100846id_/https://jira.codehaus.org/browse/BOO-1016?page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel>
