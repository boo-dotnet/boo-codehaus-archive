...
Personal preference: I would prefer to have "internationalized string literals" denoted by a # prefix. 
 ...
 Rafael Teixeira
 added a comment - 21/Aug/04 10:20 AM I also thought of it, and think we may support either GetText and ResourceManager.GetString() mechanisms, which the user chooses using command line options at compile time. 
...
Same technique can be used that is used with BOO-1 and numeric literals. 
 ...
 Doug H
 added a comment - 11/Mar/06 5:21 PM Same technique can be used that is used with BOO-1 and numeric literals.
...
Just my 2 cents, 
 ...
 Cedric Vivier
 added a comment - 09/May/07 4:46 AM I love this! 
Marking string for internationalization is such a pain in most languages and also very dependent of the implementation behind. 
We should think of an extensible API to handle this cleanly. 
...
Is there any update/progress on this? 
 ...
 Michael Dominic K.
 added a comment - 01/Feb/08 7:53 AM Is there any update/progress on this?
...
Are you willing to contribute? 
 ...
 Cedric Vivier
 added a comment - 01/Feb/08 8:10 AM Seems there is none. 
...
Let's make it for 0.9 
 ...
 Cedric Vivier
 added a comment - 12/May/08 10:41 AM Let's make it for 0.9
...
Isn't this achievable with the current feature-set, using macros or metamethods? I wouldn't want to add "noise" to the Boo core if the same functionality can be implemented externally... 
 ...
 Avishay Lavie
 added a comment - 15/Jan/09 1:35 PM Isn't this achievable with the current feature-set, using macros or metamethods? I wouldn't want to add "noise" to the Boo core if the same functionality can be implemented externally...
...
Yes, a meta method in a separate library would do just fine. 
 ...
 Rodrigo B. de Oliveira
 added a comment - 15/Jan/09 1:48 PM Yes, a meta method in a separate library would do just fine.
...
1: supporting simple strings like _"hello" is easy, but the goal here is to also support string interpolations automatically. 
 ...
 Cedric Vivier
 added a comment - 15/Jan/09 2:02 PM It's not really doable with the current feature-set with the proposed _ prefix before strings - which is a common way to mark strings for internationalization in other languages, and much more elegant than a call to some static method somewhere for each literal string). 
...
A real (ie. useful) implementation needs, as proposed in the api, a way to set a source and destination language (that can be changed at runtime, with fallback to source language if a translation for one particular string is not present), support for string interpolations etc..
 ...
 Cedric Vivier
 added a comment - 15/Jan/09 2:28 PM - edited Well yeah I guess _("foo") is a valid temporary option until our parser allows just _"foo". 
