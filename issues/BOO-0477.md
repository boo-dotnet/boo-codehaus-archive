# BOO-477: Ambiguous reference when some public propery has same name as private field

| Field | Value |
|---|---|
| Type | Bug |
| Status | Open |
| Priority | Major |
| Resolution | Unresolved |
| Reporter | Patrick Haener |
| Assignee | Rodrigo B. de Oliveira |
| Affects | None |
| Fix for | None |
| Components | None |
| Created | 2005-09-08T04:42-0500 |
| Updated | 2006-09-15T13:03-0500 |

## Description

When IKVM translates Java enums or static fields to CLI public properties and private fields with the same name are created. When trying to use the properies Boo emits an error:

....

print simfin.finance.product.Currency USD

gives:

Ambiguous reference 'USD': simfin.finance.product.Currency USD, simfin.finance.product.Currency USD.

(IronPython handles this case correctly)

## Comments (2)

### comment-46281

Hide

 Permalink

 Patrick Haener
 added a comment - 13/Sep/05 3:42 AM 

I have tried -strictfinalfieldsemantics but get the same error message. 

 Show

 Patrick Haener
 added a comment - 13/Sep/05 3:42 AM I have tried -strictfinalfieldsemantics but get the same error message.

### comment-45996

Hide

 Permalink

 Doug H
 added a comment - 08/Sep/05 8:46 PM 

I don't have access to that java lib to test, but could you try passing the -strictfinalfieldsemantics option to ikvmc and see if boo works then? I'm not sure if that will work though.

But it is strange that IKVM does that, since even C# doesn't allow you create a method or property with the same name as an existing field. 

 Show

 Doug H
 added a comment - 08/Sep/05 8:46 PM I don't have access to that java lib to test, but could you try passing the -strictfinalfieldsemantics option to ikvmc and see if boo works then? I'm not sure if that will work though. 

 But it is strange that IKVM does that, since even C# doesn't allow you create a method or property with the same name as an existing field.

## Provenance

- <http://web.archive.org/web/20150520080849id_/https://jira.codehaus.org/browse/BOO-477>
- <http://web.archive.org/web/20150521151548id_/https://jira.codehaus.org/browse/BOO-477?page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel>
