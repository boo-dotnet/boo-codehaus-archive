...
Fixed in rev. 3109 
 ...
 Cedric Vivier
 added a comment - 16/Jan/09 6:59 AM Fixed in rev. 3109
...
warning 
 ...
 Rodrigo B. de Oliveira
 added a comment - 16/Jan/09 3:47 AM warning
...
then I believe that we could emit a warning that the name is the same as a valid type and suggest the alternate syntax. 
 ...
 Marcus Griep
 added a comment - 27/Sep/07 7:57 PM There are some resolution issues in here, though, because I could define a type 'ex' that derives from System.Exception and then try to do this: 
...
What I'm saying is that sometimes the type of exception is more important than the variable used to capture it. The existing semantics require users to use a dummy variable, which is IMHO contrary to the Boo spirit, being "structure for the sake of structure". 
 ...
 Avishay Lavie
 added a comment - 12/Apr/07 6:23 PM I think it's important for Boo to support unnamed exceptions in except blocks. Think of the usual: 
...
If you want to emit a warning for that I'm cool. 
 ...
 Rodrigo B. de Oliveira
 added a comment - 12/Apr/07 5:54 PM About the: 
 And this should probably be valid as well: 
...
Let's wait the final word from Rodrigo on this though. 
 ...
 Cedric Vivier
 added a comment - 12/Apr/07 4:12 PM Yup we envision the same on this 
...
```
 ...
 Avishay Lavie
 added a comment - 12/Apr/07 4:01 PM We should definitely ensure "name" does not match the name of a valid type, or rather, if it is, to handle it as such. So these should still be valid: 
...
[1] : as far as I know the CLR does not actually have this restriction but well...
 ...
 Cedric Vivier
 added a comment - 12/Apr/07 3:32 PM Couldn't reproduce against SVN build : 
